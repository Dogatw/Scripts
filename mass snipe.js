...
// @author       You
// @match https://*/game.php?*screen=overview_villages*
// @match https://*/game.php?*screen=place*
// @icon         https://static.vecteezy.com/system/resources/thumbnails/007/756/311/small_2x/hunting-rifle-with-optical-sight-sniper-rifle-icon-vector.jpg
// @grant        none
// ==/UserScript==
...
window.sb = window.sb || window.supabase;


async function checkScriptAccess() {
    try {
        // ⏳ ensure supabase is fully ready
        ...
        return {
            allowed: data.permission === "user" || data.permission === "admin",
    ...
    function countNoblesFromMap(map) {
    let count = 0;

    map.forEach(list => {
        list.forEach(incoming => {
            const isNoble =
                incoming.labelName?.toLowerCase().includes("snob");
...
window.__CLOUD_IMPORT_OPEN__ = false;

function ensureCloudOverlay() {
    let el = document.getElementById("ms_cloud_overlay");
    if (el) return el;

    el = document.createElement("div");
    el.id = "ms_cloud_overlay";
    el.style.cssText = `
        position: fixed;
        top: 80px;
        left: 50%;
    ...
    `;
    document.documentElement.appendChild(el); // 🔑 NOT body
    return el;
}


    function hasTrain(list) {
    for (let i = 0; i < list.length; i++) {
        for (let j = i + 1; j < list.length; j++) {

           const diff = Math.abs(
    new Date(list[i].landing_time).getTime() -
    new Date(list[j].landing_time).getTime()
);

if (diff <= 200) {

                if (list[i].coord_origin === list[j].coord_origin) {
                    return true;
                }
            }
...
function isTrainSpeed(cmd) {
    return cmd.isTrain === true;
}

// ===============================
// === LZSTRING INIT (REQUIRED) ===
// ===============================
(async function initLZString() {
    ...
    console.log("✅ LZString initialized");
})();
...
let CURRENT_LOG_LEVEL = LOG_LEVEL.WARN;
...
function logOnce(key, ...args) {
    if (!logOnce._seen) logOnce._seen = new Set();
    if (logOnce._seen.has(key)) return;
    logOnce._seen.add(key);
    console.log(...args);
}

    function getSelectedSnipesFromUI() {
    const out = new Map();

    document
        .querySelectorAll(".ms_snipe_checkbox:checked")
        .forEach(cb => {
            const row = cb.closest("tr");
            if (!row) return;

            const cmdId = cb.name?.split("id_")[1];
            if (!cmdId) return;

            MAP_COMING.forEach((list, coord) => {
                list.forEach(cmd => {
                    if (cmd.commandId === cmdId) {
                        if (!out.has(coord)) out.set(coord, []);
                        out.get(coord).push(cmd);
                    }
                });
            });
        });
    ...
    // ================= EXPORT PLAN → SUPABASE (SINGLE SOURCE) =================
document.addEventListener("click", async (e) => {
    if (e.target?.id !== "btn_export_plan") return;

    console.group("📤 EXPORT PLAN");
    console.log("1️⃣ Button clicked");

    if (!window.__supabaseReady || !window.sb) {
        console.error("❌ Supabase not ready");
        UI.ErrorMessage("Cloud not ready");
        console.groupEnd();
        return;
    }
    ...
    try {
        ...
        console.log("2️⃣ World:", world, "Player:", player);
        console.log("MAP_COMING size:", MAP_COMING.size);
        console.log("MAP_TROOPS_HOME size:", MAP_TROOPS_HOME.size);

        let selectedMap = getSelectedSnipesFromUI();

// 🔁 fallback if UI is gone (overview page, reload, etc.)
if (selectedMap.size === 0 && MAP_COMING.size > 0) {
    selectedMap = new Map();

    MAP_COMING.forEach((list, coord) => {
        const nobles = list.filter(i =>
            i.labelName?.toLowerCase().includes("snob")
        );
        if (nobles.length) {
            selectedMap.set(coord, nobles);
        }
    });
}

if (selectedMap.size === 0) {
    UI.ErrorMessage("No snipes selected");
    return;
}
...
const payloadObj = [
    {
        playerName: player,
        selectedSnipes: selectedMap.size,
        totalNobles: selectedMap.size
    },
    Array.from(selectedMap.entries()),
    Array.from(MAP_TROOPS_HOME.entries())
];




        console.log("3️⃣ Payload object", payloadObj);
if (MAP_COMING.size === 0) {
    console.warn("⚠ No snipes in MAP_COMING");
    UI.ErrorMessage("No snipes to export");
    console.groupEnd();
    return;
}

      const payload = compressData(payloadObj);

if (payload) {
    console.log("4️⃣ Compressed payload length:", payload.length);
} else {
    console.warn("Compression failed", payloadObj);
}
        ...
        const { error } = await window.sb
            ...
            .upsert(
                ...
                { onConflict: "world,coord,created_by" }
            );

        if (error) {
            console.error("❌ Supabase error:", error);
            UI.ErrorMessage("Cloud save failed");
        } else {
            console.log("✅ Stored in Supabase");
            UI.SuccessMessage("Stored in cloud", 2000);
        }

    } catch (err) {
        console.error("❌ Export exception:", err);
        UI.ErrorMessage("Export failed");
    }

    console.groupEnd();
});

        // ================= IMPORT PLAN → SUPABASE (SINGLE SOURCE) =================

async function showCloudImportDialog() {
     // ⏳ wait for Supabase (prevents dialog flicker)
    while (!window.__supabaseReady || !window.sb) {
        await new Promise(r => setTimeout(r, 20));
    }

    window.__CLOUD_IMPORT_OPEN__ = true; // 🔒 lock UI rebuild

    console.group("☁️ CLOUD PLAN LIST");
    ...
    if (error) {
        ...
        UI.ErrorMessage("Cloud list failed");
        console.groupEnd();
        return;
    }

    if (!data || data.length === 0) {
        UI.InfoMessage("No cloud plans for this world");
        console.groupEnd();
        return;
    }

    // 🔑 Keep only latest plan per player
    const latestByPlayer = new Map();
    for (const row of data) {
        if (!latestByPlayer.has(row.created_by)) {
            latestByPlayer.set(row.created_by, row);
        }
    }

    renderCloudImportUI([...latestByPlayer.values()]);
...
function renderCloudImportUI(plans) {
    const box = ensureCloudOverlay();

    let html = `<b>Import latest plan from:</b><br><br>`;

    plans.forEach(row => {
        html += `
            <button style="margin:4px"
                onclick="importCloudPlanById('${row.id}')">
                ${row.created_by}
            </button>
        `;
    });
    ...
    html += `<br><br>
        <button onclick="closeCloudOverlay()">Close</button>`;

    box.innerHTML = html;
}

function closeCloudOverlay() {
    const el = document.getElementById("ms_cloud_overlay");
    if (el) el.remove();
    window.__CLOUD_IMPORT_OPEN__ = false;
}


async function importCloudPlanById(planId) {
    console.group("📥 IMPORT CLOUD PLAN", planId);

    MAP_COMING.clear();
    MAP_TROOPS_HOME.clear();
    map_selected_commandId.clear();
    ...
    if (error || !data?.payload) {
        ...
        UI.ErrorMessage("Cloud import failed");
        console.groupEnd();
        return;
    }
    ...
    let decoded;
    try {
        decoded = decompressData(data.payload);
    } catch (e) {
        console.error("❌ Payload decode failed", e);
        UI.ErrorMessage("Invalid cloud data");
        console.groupEnd();
        return;
    }

    const meta = decoded[0];
    const comingEntries = decoded[1] || [];
    const troopsHomeEntries = decoded[2] || [];

    MAP_COMING = new Map(comingEntries);
    MAP_TROOPS_HOME = new Map(troopsHomeEntries);

    total_nr_attacks = 0;

    MAP_COMING.forEach(list => {
        list.forEach(incoming => {
            if (!incoming.labelName?.includes("snob")) return;

            map_selected_commandId.set(incoming.commandId, {
                labelName: incoming.labelName,
                ignoredNoble: false
            });

            if (incoming.type_incoming?.includes("attack")) {
                total_nr_attacks++;
            }
        });
    });

    console.log("✅ Cloud plan imported from:", data.created_by);
    console.log("MAP_COMING size:", MAP_COMING.size);

    UI.SuccessMessage(
        `Imported cloud plan by ${data.created_by}`,
        2500
    );
closeCloudOverlay();
...
window.closeCloudOverlay = closeCloudOverlay;

window.showCloudImportDialog = showCloudImportDialog;
window.importCloudPlanById = importCloudPlanById;





    if (window.__MASS_SNIPE_RUNNING__) {
...
let list_href = [];
const map_selected_commandId = new Map();
...
if (typeof window.__massSnipeSpeedInit === "undefined") {
    window.__massSnipeSpeedInit = true;
...
function createMainInterface() {
      if (
        ...
        game_data.mode !== "combined"
    ) {
        console.warn(
            "createMainInterface blocked:",
            game_data.screen,
            game_data.mode
        );
        return;
    ...
    if (window.__CLOUD_IMPORT_OPEN__) {
        console.warn("UI rebuild skipped (cloud import open)");
        return;
    }
    ...
    <div id="div_container_view" class="ui-widget-content div_remove" style="width:${widthMainInterface};background-color:${backgroundColor};cursor:move;z-index:50;">
        ...
        <h2><center style="margin:10px"><u><font color="${titleColor}">${"Mass Snipes By SAM"}</font></u></center></h2>
        <center>
...
<!-- CLOUD -->
<input class="btn evt-confirm-btn btn-confirm-yes" type="button" id="btn_export_plan" style="margin:10px" value="Export Cloud">
<input class="btn evt-confirm-btn btn-confirm-yes"
       type="button"
       id="btn_import_plan"
       onclick="showCloudImportDialog()"
       style="margin:10px"
       value="Import Cloud">
...
(async function () {
    ...
    // 🔐 prevent double execution (important because of document.body.innerHTML)
    const MASS_PHASE = location.search.includes("mode=incomings")
    ? "INCOMINGS"
    : "COMBINED";

if (window.__MASS_SNIPE_ACCESS_CHECKED__ === MASS_PHASE) {
    return;
}

window.__MASS_SNIPE_ACCESS_CHECKED__ = MASS_PHASE;


    const access = await checkScriptAccess();

    if (!access.allowed) {
        UI.ErrorMessage("⛔ You are not authorized to use Mass Snipe");
        console.warn("Mass Snipe: access denied for", game_data.player.name);
        return;
    }

    // expose role globally
    window.__SCRIPT_ROLE__ = access.role;

    console.log("Mass Snipe access granted:", access.role);

  console.log("Mass Snipe init check");

if (
    game_data.screen === "overview_villages" &&
    game_data.mode === "combined"
) {
    console.log("✅ Mass Snipe UI allowed (Combined Overview)");

    createMainInterface();
    document.getElementById("btn_export_plan").disabled = MAP_COMING.size === 0;
    ...
    UI.ErrorMessage(
        "⛔ Mass Snipe works only on <b>Overview → Combined</b>.",
        8000
    );*/
}
...
//////////////////////////////////////////////////////////get supports and attacks//////////////////////////////
    ...
    const result = await extensionFetch(url);
...
function getIncomingPages(doc, incomings_href) {

    const pages = [];

    // dropdown pagination
    const visBlocks = doc.getElementsByClassName("vis");

    if (
        visBlocks.length > 1 &&
        visBlocks[1].querySelector("select")
    ) {
    ...
    // classic pagination
    else if (doc.getElementsByClassName("paged-nav-item").length) {

        doc.querySelectorAll(".paged-nav-item")
            .forEach(a => {
    ...
    // one page only
    else if (doc.getElementById("incomings_table")) {

        pages.push(incomings_href);

    }

    else {

        throw new Error("Incomings page structure not recognized");
...
async function getSupportsAndAttacks() {
...
let incomings_href =
    ...
    "overview_villages&mode=incomings&type=all&subtype=all&group=-1&page=0";
...
console.log(
    document.getElementById("incomings_table")
);

console.log(
    document.getElementsByClassName("vis").length
);

console.log(
    document.getElementsByClassName("paged-nav-item").length
);
...
// CASE 1: dropdown pagination (safe)
const visBlocks = document.getElementsByClassName("vis");
if (
    visBlocks.length > 1 &&
    visBlocks[1] &&
    visBlocks[1].getElementsByTagName("select").length > 0
) {
    const select = visBlocks[1].getElementsByTagName("select")[0];
    Array.from(select.options).forEach(opt => {
        if (opt.value) list_href.push(opt.value);
    });
...
// CASE 2: classic pagination links
else if (document.getElementsByClassName("paged-nav-item").length > 0) {
    const arr = Array.from(document.getElementsByClassName("paged-nav-item"));
    for (let i = 0; i < arr.length; i++) {
        const href = arr[i].getAttribute("href");
        if (href) list_href.push(href);
    }
}

// CASE 3: single page incomings
else if (document.getElementById("incomings_table")) {
    list_href.push(incomings_href);
}

// CASE 4: unexpected layout → hard stop (prevents silent bugs)
else {
    throw new Error("Incomings page structure not recognized");
}
...
if (!table) {
    console.warn("⚠ No incomings_table → skipping", current_url);
...
if (!tbody) {
    console.warn("⚠ No tbody → skipping", current_url);
    ajaxRequest(list_href);
    return;
}

let table_incomings = tbody.children;

                        for (let i = 1; i < table_incomings.length - 1; i++) {

                            if (table_incomings[i].children[0].innerText != "--") {

    const quick = table_incomings[i].getElementsByClassName("quickedit")[0];
    if (!quick) continue;

    const imgs = quick.getElementsByTagName("img");
    if (!imgs || imgs.length === 0) continue;


    // keep attacks + supports
if (!imgs[0].src.includes("attack") && !imgs[0].src.includes("support")) continue;

    // only filter nobles for ATTACKS, not supports
if (imgs[0].src.includes("attack")) {
    if (imgs.length < 2 || !imgs[1].src.includes("snob")) continue;
}
                                ...
                                if (map_incoming.has(coord_destination)) {
                                    ...
                                    logOnce("map-update", "map incoming updated");
                                    doc.getElementById("btn_export_plan")?.removeAttribute("disabled");
...
console.log("time ajax: " + dif_time)

// 🔧 FIX delay (no negative spam)
const delay = Math.max(200 - dif_time, 50);
...
setTimeout(() => {
    ...
    const list = map_incoming.get(key);

    // 1️⃣ village must have at least one noble
    const hasNoble = list.some(i =>
        i.labelName &&
        i.labelName.toLowerCase().includes("snob")
    );
    ...
    // ✅ TEMP list: nobles only (DO NOT store back)
    const nobleList = list.filter(i =>
        i.labelName &&
        i.labelName.toLowerCase().includes("snob")
    );

    // 2️⃣ detect train ONLY among nobles
    let hasTrain = false;

    for (let i = 0; i < nobleList.length; i++) {
        for (let j = i + 1; j < nobleList.length; j++) {
            const dt = Math.abs(
                new Date(nobleList[j].landing_time) -
                new Date(nobleList[i].landing_time)
            );

         if (
    dt <= 200 &&
    nobleList[i].coord_origin === nobleList[j].coord_origin
) {
    hasTrain = true;
}
    ...
    // mark train info on ORIGINAL list
    list.forEach(i => i.hasTrain = hasTrain);
});
...
function httpGet(theUrl) {
    ...
    return new Promise(resolve => {
        ...
        function listener(event) {
            ...
            if (event.data.type !== "bridgeResponse") return;
...
function ajaxTroopsComing(commandId) {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: game_data.link_base_pure + "info_command&id=" + commandId,
            method: "GET",
            success: (data) => {
                const doc = new DOMParser().parseFromString(data, "text/html");

                let troops = {};
                game_data.units.forEach(u => troops[u] = 0);

                const rows = doc.querySelectorAll("#attack_info_def_units tr");

                rows.forEach(tr => {
                    const img = tr.querySelector(".unit-item img");
                    if (!img) return;
                    ...
                    const unit = match[1];
                    const count = parseInt(tr.innerText) || 0;
...
async function getAllInfo() {
    ...
    const map_coming = await getSupportsAndAttacks();
    const map_troops_home = new Map();
    const list_sniped_coord = new Set();

    // after data is fully built
    MAP_COMING = map_coming;
    MAP_TROOPS_HOME = map_troops_home;
   ...
   ///////////////////////////////////////////
// get info troops home ONLY (no supports)
///////////////////////////////////////////
const run = async () => {
    console.log("Starting (nobles only)...");

    for (let i = 0; i < keys.length; i++) { // for each coord
        const list_coming = map_coming.get(keys[i]);

        // ✅ CRITICAL FIX: sort by landing time
list_coming.sort((a, b) =>
    new Date(a.landing_time) - new Date(b.landing_time)
);

        for (let j = 0; j < list_coming.length; j++) { // for each incoming
            const incoming = list_coming[j];

            // ✅ FIX 2: fetch relevant support troops (only near nobles)
if (incoming.type_incoming.includes("support")) {
    for (let k = j + 1; k < list_coming.length; k++) {
        const next = list_coming[k];

        const dt =
            new Date(next.landing_time) -
            new Date(incoming.landing_time);

        if (dt > 200) break;

        if (next.labelName?.toLowerCase().includes("snob")) {
            try {
                const troops = await ajaxTroopsComing(incoming.commandId);
                incoming.troops = troops;
            } catch (e) {
                console.log("support troop fetch failed", e);
            }
            break;
        }
    }
            ...
            // 🔥 IGNORE SUPPORTS COMPLETELY
            if (!incoming.type_incoming.includes("attack")) {
                continue;
            }

            const coord = incoming.coord_destination;
            const villageId = incoming.coord_destination_id;

            // get troops home ONCE per village
            if (!map_troops_home.has(coord)) {
                try {
                    const obj = await ajaxTroopsStationed(villageId);

                    const serverTime =
                        document.getElementById("serverTime").innerText;
                    let serverDate =
                        document.getElementById("serverDate").innerText.split("/");
                    serverDate =
                        serverDate[1] +
                        "/" +
                        serverDate[0] +
                    ...
                    UI.SuccessMessage(
                        `info coord: ${keys.length - i}, get troops home ${
                            list_coming.length - j
                        }`
                    );
                } catch (error) {
                    console.log("troops home error", error);
                }
            }
        }
    ...
    console.log("Done (supports removed).");
};

await run();

//////////////////////////adding sniped train to sniped list////////////////////

const threshold = parseInt(
    document.getElementById("already_sniped_pop").value || 3000
);
...
map_coming.forEach((list, coord) => {
    ...
    // village doesn't even contain a train
    if (!hasTrain(list))
        return;
    ...
    for (let i = 0; i < list.length; i++) {
        ...
        if (!first.labelName?.includes("snob"))
            continue;
        ...
        for (let j = i + 1; j < list.length; j++) {
            ...
            const diff =
                new Date(cmd.landing_time) -
                new Date(first.landing_time);

            if (diff > 200)
                break;

            if (cmd.type_incoming.includes("support") && cmd.troops) {

                Object.entries(cmd.troops).forEach(([unit, amount])=>{
                    supportPop += (troopsPop[unit] || 0) * amount;
                });
            }
        }

        if (supportPop >= threshold) {
            list_sniped_coord.add(coord);
            break;
        }
    }
...
if (
    table_incomings &&
    table_incomings.children.length > 1 &&
    table_incomings.children[1].children.length > 1
) {
    lastRow = table_incomings.children[1].children[1];
    lastRow.remove();
}
...
Array.from(map_coming.keys()).forEach(key => {
    ...
    for (let i = 0; i < list_coming.length; i++) {
        ...
        // 🚫 GET TRAINS UI — SHOW ONLY NOBLE SPEED
        if (
            !list_coming[i].labelName ||
            !list_coming[i].labelName.toLowerCase().includes("snob")
        ) {
            continue;
        }
...
const headerCell = table_incomings.querySelector(
    "thead th, thead td"
);

if (headerCell) {
    headerCell.innerText = `Command (${nr_row})`;
}
    if (lastRow) {
    ...
    /////////////////////////////////////highlight trains///////////////////////////
     table_incomings = document.getElementById("incomings_table").getElementsByTagName("tr")
...
if (window.CommandsOverview?.init) {
    window.CommandsOverview.init();
}
    ...
    const cmd = list_incoming[i];

    // ✅ ALWAYS keep supports
    if (cmd.type_incoming.includes("support")) {
        list_result.push(cmd);
        continue;
    }

    // ✅ keep only selected nobles
    if (map_selected_commandId.has(cmd.commandId)) {
        let obj_selected = map_selected_commandId.get(cmd.commandId);

        delete cmd.tr;
        cmd.labelName = obj_selected.labelName;

        list_result.push(cmd);
        commandIdExist = true;

        if (obj_selected.ignoredNoble == true) {
            ignoredNoble = true;
        }
    }

    if (cmd.type_incoming.includes("attack"))
        total_nr_attacks++;
}
            if (commandIdExist == true && ignoredNoble == false) { //update map with only checked incoming
                ...
                // attach to FIRST NOBLE instead of index 0
const firstNoble = list_result.find(i =>
    i.labelName && i.labelName.toLowerCase().includes("snob")
);

if (firstNoble) {
    firstNoble.total_nr_attacks = total_nr_attacks;
}
        ...
        const box = document.getElementById("list_sniped_coord");

if (box) {
    box.value = [...list_sniped_coord].join("\n");
}
    })
...
// ================= AUTO MASS CONTROLLER =================
async function AUTO_RUN_MASS() {
    if (window.__AUTO_RUNNING__) {
        console.log("⏳ Auto already running, skip");
        return;
    }
    ...
    try {
        // STEP 1: scan incomings
        const map = await getSupportsAndAttacks();
        MAP_COMING = map;

        // STEP 2: auto-select ALL nobles (no UI needed)
        map_selected_commandId.clear();

        MAP_COMING.forEach((list) => {
            list.forEach(cmd => {
                if (cmd.labelName?.toLowerCase().includes("snob")) {
                    map_selected_commandId.set(cmd.commandId, {
        ...
        console.log("✅ Auto-selected nobles:", map_selected_commandId.size);

        // STEP 3: fetch troops + process
        await getAllInfo();
...
function counterTime() {
    ...
    idSetInteral = window.setInterval(() => {
        ...
        $(".counterTime").each((index, item) => {
            ...
            let time = parseInt(item.getAttribute("date-time"), 10);

            if (date_current > time) {
                item.innerText = "done";
            } else {
                let hours = ("0" + Math.floor((time - date_current) / (3600 * 1000))).slice(-3);
                let minutes = ("0" + Math.floor(((time - date_current) / (60 * 1000)) % 60)).slice(-2);
                let seconds = ("0" + Math.floor(((time - date_current) / 1000) % 60)).slice(-2);
...
function isRamOrCatSpeed(cmd) {
    return (
        cmd.labelName &&
        (cmd.labelName.includes("ram") || cmd.labelName.includes("catapult"))
    );
}


function getTroopsSnipes(snipe_pop) {
if (!game_data || game_data.screen !== "overview_villages") {
    UI.ErrorMessage("Please run this from the Combined Overview");
    return [];
}
...
if (!combinedTable) {
    UI.ErrorMessage("Combined overview not available on this screen");
    return [];
}

let table_combined = combinedTable.getElementsByTagName("tr");
    let units = game_data.units
...
function createTableViewIncomings(map_troops_home, map_coming, map_possible_snipes) {
    ...
    Array.from(map_coming.keys()).forEach(key => {
        ...
        for (let i = 0; i < list_coming.length; i++) {
            if (list_coming[i].labelName.includes("snob")) {
                for (let j = i + 1; j < list_coming.length; j++) {
                    let difTime = new Date(list_coming[j].landing_time).getTime() - new Date(list_coming[i].landing_time).getTime() // diff between 2 dates
                    if (difTime > 0 && difTime <= 200 && list_coming[j].labelName.includes("snob")) { // i have a train
                        landing_time = list_coming[i].landing_time
                        break;
                    }
                }
   ...
   idInterval_alarm = window.setInterval(() => {
    const arrived_vect = Array.from($(".alarmSound"));

    for (let i = 0; i < arrived_vect.length; i++) {
        const el = arrived_vect[i];
        const tr = el.closest("tr");
        if (!tr) continue;

        // HH:MM:SS format from counter
        const split_time = el.innerText.split(":");
        if (split_time.length < 3) continue;

        const total_time =
            parseInt(split_time[0]) * 3600 * 1000 +
            parseInt(split_time[1]) * 60 * 1000 +
            parseInt(split_time[2]) * 1000;
        ...
        if (total_time < alarm_sound) {
            ...
            // 🚀 AUTO SEND
            const sendBtn = tr.querySelector(".go_to_rally");
            if (!sendBtn) continue;

            const coord = sendBtn.getAttribute("coord-dest");
            const launchTime = tr.children[6]?.innerText || "";
            const key = coord + "|" + launchTime;

            if (AUTO_SENT.has(key)) continue; // already sent

            AUTO_SENT.add(key);
            console.log("🚀 AUTO SEND", key);
...
window.createTableViewIncomings=createTableViewIncomings;
...
function createTableCoordTroops(obj_troops_home, list_coming) {
    ...
    for (let i = 0; i < list_coming.length; i++) {
    const cmd = list_coming[i];

    if (cmd.type_incoming.includes("support") && cmd.troops) {
        Object.keys(troopsSupportComing).forEach(key => {
            troopsSupportComing[key] += (cmd.troops[key] || 0);
        });
    }
}
...
window.createTableCoordTroops=createTableCoordTroops;
...
// nobles-only rendering (supports removed)

if (labelName && labelName.includes("snob")) {
    headerNoble = "#3f0000";      // red background
    titleColorNoble = "#4dff4d";  // green ms
}
...
window.createTableCoordIncomings=createTableCoordIncomings;
...
window.createTableImport=createTableImport;
...
window.createTableExport=createTableExport;
...
async function saveSnipesToSupabase(world, coord, playerName, payloadObj) {
    if (!window.supabase) {
        console.warn("Supabase not ready");
        return;
    }

    const compressed = compressData(payloadObj);
    ...
    if (error) {
        ...
        UI.SuccessMessage("Snipes saved to cloud");
    }
}
...
const LANDING_GROUP_MS = 500;

function getLandingAnchorMs(landing_time) {
    return Math.floor(new Date(landing_time).getTime() / LANDING_GROUP_MS) * LANDING_GROUP_MS;
}
            ...
            if (landing_time > date_current) {
                ...
                if (list_coming[i].labelName.includes("snob")) {
                    ...
                    for (let j = i + 1; j < list_coming.length; j++) { //check if there is another noble at 50,100,150ms away
                        ...
                        const delta = landing_time_next - landing_time;
...
if (delta > 0 && delta <= 200) {
                            if (list_coming[i].coord_origin == list_coming[j].coord_origin) //there is a train
                            {
                                ...
                                if (pop > alread_pop_sniped) { //it is already sniped
                                    ...
                                    const landingKey = getLandingAnchorMs(list_coming[i].landing_time);

if (!map_coming_snipes.has(landingKey)) {
    map_coming_snipes.set(landingKey, {
        coord_destination: list_coming[i].coord_destination,
        coord_destination_id: list_coming[i].coord_destination_id,
        landing_time: list_coming[i].landing_time, // first command = anchor
    ...
    Array.from(map_coming_snipes.keys()).forEach(key => {
        ...
        for (let i = 0; i < list_available.length; i++) {
            ...
            // Reserve swords for sword-speed snipes
if (availableTroupes.sword < 10) {
    availableTroupes.sword = 0;
}
...
window.startSnipes=startSnipes;
...
function createTableSnipes(list_snipes, map_possible_snipes, obj_snipes_total) {
    ...
    $("#table_send_snipes").find(".go_to_rally").on("click", (event) => {
        // ================= SAVE ARRIVAL TIME FOR CONFIRM SCRIPT =================
try {
    const tr = $(event.target).closest("tr");

    // landing time text: "02/01/2026 21:05:26 569"
    const landingText = tr
        .find("td")
        .eq(7)
        .text()
        .trim();

    const match = landingText.match(
        /(\d{2}:\d{2}:\d{2})\s+(\d{3})/
    );
    ...
    if (match) {
        sessionStorage.setItem(
            "tw_mass_arrival",
            JSON.stringify({
                hms: match[1],      // HH:MM:SS
                ms: parseInt(match[2], 10)
            })
        );
        console.log("🟢 MassSnipe arrival stored", match[1], match[2]);
    }
} catch (e) {
    console.warn("Arrival time capture failed", e);
}
...
window.showIncoming=showIncoming;
...
(function autoCloseAfterConfirm() {

    if (!sessionStorage.getItem("MS_ATTACK_CONFIRMED")) return;

    if (
        location.search.includes("screen=place") &&
        !location.search.includes("try=confirm")
    ) {
        sessionStorage.removeItem("MS_ATTACK_CONFIRMED");
...
(function autoSupport() {
    ...
    if (!location.href.includes("screen=place") || location.href.includes("try=confirm")) return;

    if (sessionStorage.getItem("AUTO_SUPPORT_SENT")) return;
    sessionStorage.setItem("AUTO_SUPPORT_SENT", "1");
    ...
    setTimeout(() => {
        ...
        const btn =
            document.querySelector("#target_support") ||
            document.querySelector("input[name='support']") ||
            document.querySelector(".btn-support");

        if (!btn) {
            console.warn("❌ Support not found");
            return;
        }

        console.log("🛡️ CLICK SUPPORT");
...
(function autoConfirmclick() {
    // run ONLY on rally page
    if (!location.href.includes("screen=place&try=confirm")) return;
    'use strict';

    const TF_CONFIRM_LOCK = "tf_confirm_sent";

    /* ================= NOBLE SPLIT CONFIG ================= */

    const EXTRA_ATTACKS_IF_NOBLE = 4;
    const NOBLE_SPLIT_DONE = "tw_noble_split_done";

    /* ================= FIXED ARRIVAL CONFIG ================= */
    function getArrivalFromMassSnipe() {
    try {
        const data = sessionStorage.getItem("tw_mass_arrival");
        if (!data) return null;
...
// ================= HARD-CODED MS OFFSET =================
const HARD_MS_OFFSET = 20; // ← change this number only



    const NOBLE_CONFIRM_DELAY_MS = 40;


    /* ================= STORAGE ================= */
    const NOBLE_DELAY_APPLIED = "tw_noble_delay_applied";
    ...
    function clearState() {
        ...
        sessionStorage.removeItem(NOBLE_SPLIT_DONE);
        sessionStorage.removeItem(NOBLE_DELAY_APPLIED);
    ...
    function hasNobleInCommand() {
        const nobleCell = document.querySelector(
            '#command-data-form span.unit_snob, ' +
            '#command-data-form input[name*="snob"]'
        );

        if (!nobleCell) return false;

        const count = parseInt(
            nobleCell.textContent || nobleCell.value || "0",
            10
        );
    ...
    function addAdditionalAttacksIfNeeded(done) {
        if (sessionStorage.getItem(NOBLE_SPLIT_DONE)) {
            done();
            return;
        }

        if (!hasNobleInCommand()) {
            done();
            return;
        }

        const btn = document.getElementById("troop_confirm_train");
        if (!btn) {
            console.warn("Noble detected but add-attack button missing");
            done();
            return;
        }
        ...
        const iv = setInterval(() => {
            ...
            if (i >= EXTRA_ATTACKS_IF_NOBLE) {
                clearInterval(iv);
                sessionStorage.setItem(NOBLE_SPLIT_DONE, "1");
                setTimeout(done, 150);
            }
        }, 150);
    }
    ...
    /* ================= AUTO ARM FIXED ARRIVAL ================= */

(function autoArm() {
    const fromMass = getArrivalFromMassSnipe();
    if (!fromMass) {
        console.warn("⛔ No MassSnipe arrival found");
        return;
    }
...
delay = Math.max(0, delayTime + inputMs + HARD_MS_OFFSET);
    ...
    function scheduleReloadBeforeArrival(arrival) {
        const watcher = setInterval(() => {
            ...
            const target = toSeconds(arrival) - getReloadBuffer();
    ...
    function setArrivalTime() {
        ...
        arrInterval = setInterval(() => {
            ...
            if (arr.slice(-8) >= input) {
             ...
             setTimeout(() => {
    sessionStorage.setItem("MS_ATTACK_CONFIRMED", "1");

    document.getElementById("troop_confirm_submit").click();
    sessionStorage.setItem(TF_CONFIRM_LOCK, "1");
...
delay = state.delay + HARD_MS_OFFSET;

    document.getElementById("showArrTime").textContent =
    `${input}:${(inputMs + HARD_MS_OFFSET).toString().padStart(3, "0")}`;


    if (sessionStorage.getItem(RELOAD_FLAG) === "1") {

        // ✅ Apply noble confirm delay ONCE
        if (
            hasNobleInCommand() &&
            !sessionStorage.getItem(NOBLE_DELAY_APPLIED)
        ) {
            delay += NOBLE_CONFIRM_DELAY_MS;
            sessionStorage.setItem(NOBLE_DELAY_APPLIED, "1");
        }

        addAdditionalAttacksIfNeeded(setArrivalTime);
    }
})();
...
(function initAutoLoop() {
    ...
    console.log("⏱ Auto loop initialized");

    const INTERVAL = (30 * 60 + Math.random() * 120) * 1000; // 30 min
